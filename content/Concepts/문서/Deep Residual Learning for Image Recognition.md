...
## Residual Network
Plain Network를 기반으로, Shortcut Connections를 도입한다.
Identity Shortcut은 입력과 출력이 같은 차원일 때 바로 사용될 수 있다. [^5] 차원이 증가하면 [^6] 두가지 옵션을 고려할 수 있다.
1. Shortcut은 여전히 항등매핑을 수행한다. 추가된 차원에 대해서는 제로패딩을 한다. 이러한 옵션에는 추가적인 파라미터가 필요하지 않다.
2. 방정식 $\mathbf{y} = \mathcal{F}(\mathbf{x}, \{W_i\}) + W_s\mathbf{x}$이 차원을 맞추기 위해 사용될 수 있다. 이는 1x1의 컨볼루션으로 구현된다.
두가지 선택지 모두, 지름길이 서로 다른 크기의 feature map을 가로지를 경우에는 2의 [[Stride|스트라이드]]로 수행된다.
...
# 구현
본 논문의 ImageNet구현은 기존 CNN학습에서 널리 사용되는 데이터 증강 및 학습 절차를 기반으로 한다. AlexNet에서 제안된 입력 전처리 및 [[Stochastic Gradient Descent|SGD]]기반 학습 방식과 VGG에서 제안된 다중 스케일 처리 방식을 따른다.
구체적으로, 입력 이미지는 데이터 다양성을 확보하기 위해 짧은 변의 길이를 일정 범위 내에서 무작위로 선택하여 리사이즈하는 스케일 증강을 수행한다. 이후, 이미지 또는 수평 반전된 이미지)로부터 224x224크기의 패치를 무작위로 추출하고, 각 픽셀에서 데이터셋 평균값을 차감하는 방식의 전처리를 적용한다. 또한 RGB채널 값에 변화를 주는 색상 증강을 통해 모델의 일반화 성능을 향상시킨다.
최적화는 mini-batch 크기 256을 사용하는 [[Stochastic Gradient Descent|확률적 경사하강법]]으로 수행되며, 초기 학습률을 0.1로 설정하고, 학습 과정에서 오차 감소가 정체되는 시점마다 학습률을 1/10으로 감소시키는 방식의 step decay스케줄을 적용한다. 전체 학습은 약 60만 iteration동안 진행된다.
정규화를 통해 weight decay(0.0001)와 momentum(0.9)를 사용하며, 별도의 dropout은 적용하지 않는다. 이는 [[Batch Normalization]]자체가 일정 수준의 정규화 효과를 제공하기 때문이다.
테스트 단계에서는 입력 이미지의 여러 위치에서 crop을 수행하여 각각의 예측을 평균내는 다중 크롭 평가 방식을 사용한다. 또한 모델을 Fully Convolutional 형태로 변환하여 다양한 해상도의 입력 이미지에 대해 예측을 수행하고, 여러 스케일에서 얻은 결과를 평균하는 다중 스케일 평가 전략을 적용한다.

# 실험
## 1. ImageNet 분류
ImageNet 2012 분류 데이터셋 (1000개의 클래스)에서 측정했다. 모델은 1.28M개의 훈련용 이미지로 훈련되었고, 50K개의 검증용 데이터로 평가되었다. 100K개의 테스트 데이터로 최종 평가를 하였다. [[Top-k Accuracy|Top1과 Top-5 에러율]]을 평가한다.
### Plain Network
18-layer와 34-layer 플레인 네트워크를 평가했다. 깊은(34)모델이 얕은 모델보다 더 큰 검증 오류를 보여줬다. 이유를 밝히기 위해, 훈련 과정 동안의 훈련/검증 에러를 비교했다. [[degradation]]문제를 발견했다. (34넷이 항상 18넷보다 *훈련*오류가 높았다. 이론적으로 34넷은 18넷의 모든 기능을 수행할 수 있음에도 불구하고)
저자들은 이러한 최적화의 어려움이 [[Gradient Vanishing|기울기 소실]]에 의하였을 가능성은 낮다는 주장을 한다. 이 플레인넷은 [[Batch Normalization]]으로 학습되었다. BN은 정방향 전파된 신호가 0이 아닌 다양성을 지닐것을 보장한다. (포워드 시그널이 항상 적당한 다양성을 유지하는것은 Gradient도 죽지 않는다는 상당한 상관관계를 가진다) 우리는 또한 역전파된 기울기가 정상적인 노름을 지닌다는것을 확실히 했다. 따라서 정방향/역방향 신호는 소실되지 않았다. 실제로, 34층 플레인넷은 꽤 괜찮은 정확성 결과를 냈다. 이는 솔버가 어느정도 잘 작동한다는것을 의미한다. 깊은 플레인넷은 아마도 기하급수적인 낮은 수렴률을 가지고 있을것으로 추측한다. 이는 [[Training Error and Test Error|훈련 에러]]와 연관이 있다.


# 질문
## ResNet이후에는 깊을수록 더 좋은 모델인가?
그건 아니다. ResNet이전에는 깊어지면 오히려 망가지는 [[degradation]]문제가 있었다. ResNet이 해결하는것은 이론적으로 "적어도 나빠지진 않아야 한다" 까지이고(항등매핑된 층이 있다면) 그 다음은 뭐 없다.
...
[^5]: [[ResNet Paper Fig3 Network Architectures.png|그림]] 우측의 실선 화살표
[^6]: 그림에서의 점선